...
], function (Map, SceneView, CSVLayer, FeatureLayer, Basemap, reactiveUtils) {
  const main = async () => {
    ...
    const formatDateTime = (date) => {
      return date.toISOString().slice(0, 19).replace('T', '%20');
    };

    const response = await fetch(`https://earthquake.usgs.gov/fdsnws/event/1/query.csv?starttime=${formatDateTime(startDate)}&endtime=${formatDateTime(endDate)}&minmagnitude=4&orderby=time`);
        ...
        visualVariables: [
          {
            ...
            axis: "all",
            stops: [
              { value: 5.5, size: 100000, label: "5.5" },
              { value: 7, size: 450000, label: "7" }
            ]
          },
          {
            type: "color",
    ...
    earthquakeLayer
      ...
      .then(function (result) {
        const earthquakeNo = result.features.length;
        document.getElementById("earthquake-number").innerHTML = earthquakeNo;
    ...
    view.when(function () {
      ...
      reactiveUtils.once(() => !view.updating, function () {
        rotate();
      });
    });
